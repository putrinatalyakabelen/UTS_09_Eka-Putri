...
// class BillCalenderScreen extends StatelessWidget {
//   const BillCalenderScreen({super.key});
...
class BillCalenderScreen extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      backgroundColor: Colors.grey[100],
      appBar: AppBar(
        title: const Text('Tagihan Kelas', style: TextStyle(color: Colors.white)),
        centerTitle: true,
        backgroundColor: Colors.teal,
        elevation: 2,
        actions: [
          ...
          return const Center(
            child: Column(
              ...
              children: [
                Icon(Icons.receipt_long, size: 64, color: Colors.grey),
                SizedBox(height: 16),
                Text('Belum ada tagihan', style: TextStyle(fontSize: 18, color: Colors.grey)),
                ...
                onTap: () async {
                  final confirm = await Get.dialog(
                    AlertDialog(
                      title: Text(bill.name),
                      content: Text(
                        bill.isPaid ? 'Tagihan sudah lunas.' : 'Tandai sebagai lunas?',
                      ),
                      actions: [
                        TextButton(
                          onPressed: () => Get.back(result: false),
